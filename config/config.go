...
type Config struct {
	...
	AgentTools  AgentToolsConfig  `yaml:"agent_tools"`    // 智能体工具注册/限流等
...
type AgentToolsConfig struct {
	EnableAudit bool `yaml:"enable_audit"` // 是否开启调用审计
}
...
func loadFromEnv(cfg *Config) {
	// Server配置
	if port := os.Getenv("COREX_SERVER_PORT"); port != "" {
		if p, err := strconv.Atoi(port); err == nil {
			cfg.Server.Port = p
		}
	}
	if mode := os.Getenv("COREX_SERVER_MODE"); mode != "" {
		cfg.Server.Mode = mode
	}
	if timeout := os.Getenv("COREX_SERVER_READ_TIMEOUT"); timeout != "" {
		if t, err := strconv.Atoi(timeout); err == nil {
			cfg.Server.ReadTimeoutSeconds = t
		}
	}
	if timeout := os.Getenv("COREX_SERVER_WRITE_TIMEOUT"); timeout != "" {
		if t, err := strconv.Atoi(timeout); err == nil {
	...
	// Auth配置
	if secret := os.Getenv("COREX_AUTH_JWT_SECRET"); secret != "" {
		cfg.Auth.JWTSecret = secret
	}
	if audience := os.Getenv("COREX_AUTH_EXPECTED_AUDIENCE"); audience != "" {
		cfg.Auth.ExpectedAudience = audience
	}
	if scopes := os.Getenv("COREX_AUTH_REQUIRED_SCOPES"); scopes != "" {
		cfg.Auth.RequiredScopes = strings.Split(scopes, ",")
	}
	if ttl := os.Getenv("COREX_AUTH_TOKEN_TTL_HOURS"); ttl != "" {
	...
	// EventBus配置
	if busType := os.Getenv("COREX_EVENT_BUS_TYPE"); busType != "" {
		cfg.EventBus.Type = busType
	}
	if redisAddr := os.Getenv("COREX_EVENT_BUS_REDIS_ADDR"); redisAddr != "" {
		cfg.EventBus.RedisAddr = redisAddr
	}
	if redisPassword := os.Getenv("COREX_EVENT_BUS_REDIS_PASSWORD"); redisPassword != "" {
		cfg.EventBus.RedisPassword = redisPassword
	}
	if ttl := os.Getenv("COREX_EVENT_BUS_DEDUPE_TTL_SEC"); ttl != "" {
		if t, err := strconv.Atoi(ttl); err == nil {
	...
	// LowCode配置
	if maxFlows := os.Getenv("COREX_LOW_CODE_MAX_CONCURRENT_FLOWS"); maxFlows != "" {
		if m, err := strconv.Atoi(maxFlows); err == nil {
			cfg.LowCode.MaxConcurrentFlows = m
		}
	}
	if timeout := os.Getenv("COREX_LOW_CODE_DEFAULT_TIMEOUT_SEC"); timeout != "" {
		if t, err := strconv.Atoi(timeout); err == nil {
	...
	// AgentTools配置
	if audit := os.Getenv("COREX_AGENT_TOOLS_ENABLE_AUDIT"); audit != "" {
	...
	// FeatureGate配置
	if license := os.Getenv("COREX_FEATURE_GATE_LICENSE_KEY"); license != "" {
